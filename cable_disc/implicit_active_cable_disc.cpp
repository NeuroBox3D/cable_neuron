...
#include "implicit_active_cable_disc.h"
...
ImplicitActiveCableDisc<TDomain>::
ImplicitActiveCableDisc(const char* functions, const char* subsets)
: ImplicitActiveCableDiscBase<TDomain>(functions, subsets)
{}


template<typename TDomain>
void ImplicitActiveCableDisc<TDomain>::
prepare_setting(const std::vector<LFEID>& vLfeID, bool bNonRegularGrid)
{
	// check number of unknown functions
	if (vLfeID.size() != 4)
		UG_THROW("ImplicitActiveCableDisc: Wrong number of functions given. Need exactly "<< 4);
...
// channel kinetics
	for (size_t ip = 0; ip < geo.num_scv(); ++ip)
	{
		...
		const number potassium_part_of_flux = m_gK[ip] * pow(n,4) * (vm - m_eK[ip]);
		const number sodium_part_of_flux =  m_gNa[ip] * pow(m,3) * h * (vm - m_eNa[ip]);
...
// cable equation, axial current part
...
// channel kinetics derivatives
	for (size_t ip = 0; ip < geo.num_scv(); ++ip)
	{
		...
		J(_vm_, co, _h_, co) += scv.volume()*PI*diam * m_gNa[ip]*pow(m,3) * (vm - m_eNa[ip]);
		J(_vm_, co, _m_, co) += scv.volume()*PI*diam * 3.0*m_gNa[ip]*pow(m,2) * h * (vm - m_eNa[ip]);
		J(_vm_, co, _n_, co) += scv.volume()*PI*diam * 4.0*m_gK[ip]*pow(n,3) * (vm - m_eK[ip]);
		J(_vm_, co, _vm_, co) += scv.volume()*PI*diam * (m_gK[ip]*pow(n,4) + m_gNa[ip]*pow(m,3)*h + m_gL[ip]);
	}

// axial current derivatives
...
#ifdef UG_DIM_3
template class ImplicitActiveCableDisc<Domain3d>;
#endif
