...
ElemDiscHH<TDomain>::
ElemDiscHH(const char* functions, const char* subsets)
...
template<typename TDomain>
void ElemDiscHH<TDomain>::
prepare_setting(const std::vector<LFEID>& vLfeID, bool bNonRegularGrid)
{
//	check number
	// 4 funktionen Vm, h, m, n
	if(vLfeID.size() != 4)
		UG_THROW("ElemDiscHH: Wrong number of functions given. "
				"Need exactly "<<4);

	if(vLfeID[0].order() != 1 || vLfeID[0].type() != LFEID::LAGRANGE)
		UG_THROW("ElemDisc FV Scheme only implemented for 1st order.");
...
//	check, that upwind has been set
	if(m_spConvShape.invalid())
		UG_THROW("KabelDiffFV::prep_elem_loop:"
...
//	set local positions
	if(TFVGeom::usesHangingNodes)
	{
		...
		if(m_spConvShape.valid())
			if(!m_spConvShape->template set_geometry_type<TFVGeom>(geo))
				UG_THROW("ElemDiscHH::prep_elem_loop:"
								" Cannot init upwind for element type.");
	}
...
// 	loop Sub Control Volumes (SCV)
	...
	// 	Add to local matrix
	//	Here m,n,h values
		double AlphaHn = ((0.01*(sol(_VM_,id) + 55))/(1-exp(-0.1*(sol(_VM_,id)+55))));
		double BetaHn  = ((0.125*exp(-0.0125*(sol(_VM_,id)+65))));
		double AlphaHm = ((0.1*(sol(_VM_,id)+40))/(1-exp(-0.1*(sol(_VM_,id)+40))));
		double BetaHm  = (4*exp(-0.0556*(sol(_VM_,id)+65)));
		double AlphaHh = (0.07*exp(-0.05*(sol(_VM_,id)+65)));
		double BetaHh  = (1/(1 + exp(-0.1*(sol(_VM_,id)+35))));
		J(_h_, co, _h_, co) += scv.volume() * AlphaHh * (1-sol(_h_,id)) - BetaHh*(sol(_h_,id));
		J(_m_, co, _m_, co) += scv.volume() * AlphaHm * (1-sol(_m_,id)) - BetaHh*(sol(_m_,id));
		J(_n_, co, _n_, co) += scv.volume() * AlphaHn * (1-sol(_n_,id)) - BetaHh*(sol(_n_,id));
	}
...
//	set local positions
	if (!TFVGeom::usesHangingNodes)
	{
		...
		TFVGeom& geo = GeomProvider<TFVGeom>::get();
		if (!m_spConvShape->template set_geometry_type<TFVGeom>(geo))
			UG_THROW("KabelDiffFV::prep_err_est_elem_loop: "
					 "Cannot init upwind for element type.");
...
// 	update geometry for this element
	...
	UG_CATCH_THROW("KabelDiffFV::prep_err_est_elem: Cannot update Finite Volume Geometry.");
...
//	computes the linearized defect w.r.t to the velocity
template<typename TDomain>
const typename ElemDiscHH<TDomain>::conv_shape_type&
ElemDiscHH<TDomain>::
...
#ifdef UG_DIM_1
template class ElemDiscHH<Domain1d>;
#endif
#ifdef UG_DIM_2
template class ElemDiscHH<Domain2d>;
#endif
#ifdef UG_DIM_3
template class ElemDiscHH<Domain3d>;
#endif
