#ifndef ca_converted_allNernst_UG_H_
#define ca_converted_allNernst_UG_H_
...
ca_converted_allNernst_UG(const char* functions, const char* subsets) 
...
m_log_mGate(false), 
m_log_hGate(false) {} 
UG_CATCH_THROW("Error in ca_converted_allNernst_UG initializer list. "); 
 
 
/// @copydoc ICableMembraneTransport<TDomain>::ICableMembraneTransport(const std::vector<std::string>&) 
ca_converted_allNernst_UG(const std::vector<std::string>& functions, const std::vector<std::string>& subsets) 
try : ICableMembraneTransport<TDomain>(functions, subsets), 
	gbar ( 0.1   	*10), 
	vshift ( 0	*1), 
	cao  ( 2.5	*1), 
...
virtual ~ca_converted_allNernst_UG() {}; 
...
#endif // ca_converted_allNernst_UG_H_
