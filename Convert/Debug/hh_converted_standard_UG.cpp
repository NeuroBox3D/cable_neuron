...
template<typename TDomain> 
void hh_converted_standard_UG<TDomain>::ce_obj_available()  
{  
	init_attachments();  
 	F = m_pCE->F; 
 R = m_pCE->R; 
...
template<typename TDomain> 
void hh_converted_standard_UG<TDomain>::current(Vertex* ver, const std::vector<number>& vrt_values, std::vector<number>& outCurrentValues) 
{ 
...
number ena; 
if (m_pCE->rev_pot_na() == 0) 
{ 
	  ena = helpV*(log(m_pCE->na_out()/na)); 
} 
else 
{ 
	  ena = m_pCE->rev_pot_na(); 
} 
number ek; 
if (m_pCE->rev_pot_k() == 0) 
{ 
	  ek = helpV*(log(m_pCE->na_out()/na)); 
} 
else 
{ 
	  ek = m_pCE->rev_pot_k(); 
