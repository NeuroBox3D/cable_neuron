...
double depp::vtrap(double x, double y, double z, double d) 
{ 
        if (fabs(x/y) < 1e-6) {
                return  y*(1 - x/y/2)
        }else{
...
void depp<TDomain, TAlgebra>::init(number time, SmartPtr<GridFunction<TDomain, TAlgebra> > spGridFct) 
{ 
// attach attachments 
...
template<typename TDomain, typename TAlgebra> 
void depp<TDomain, TAlgebra>::update_gating(number newTime, SmartPtr<GridFunction<TDomain, TAlgebra> > spGridFct) 
{ 
typedef typename DoFDistribution::traits<Vertex>::const_iterator itType; 
SubsetGroup ssGrp; 
try { ssGrp = SubsetGroup(spGridFct->domain()->subset_handler(), this->m_vSubset);} 
...
double         m +=  (minf-m)/mtau*dt; 
double         h += (hinf-h)/htau*dt; 
double         n += (ninf-n)/ntau*dt; 
...
template<typename TDomain, typename TAlgebra> 
void depp<TDomain, TAlgebra>::ionic_current(Vertex* ver, std::vector<number>& outCurrentValues) 
{ 
...
outCurrentValues.push_back(/m_F ) 
